...
function renderProductInMyCart(cartProduct) {
  ...
  cartProduct.map((productItem) => {
    ...
    const removeButton = itemElement.querySelector(
      `#remove-from-cart-${productItem.id}`
    );
    removeButton.addEventListener("click", () =>
      removeCartItem(productItem.id)
    );
...
function renderAllPaymentDescForCheckout(cartProduct) {
  ...
  checkout.innerHTML += ` <div class="item-total-price">
                                    ...
                                    <span class="total-price">$${totalPrice.toFixed(
                                      2
                                    )}</span>
                                </div>
                                `;
  ...
  cartBtnForCheckout.addEventListener("click", (e) =>
    paymentThroughRazorpay(e)
  );
}
